            ...
            <TableBody>
              {
                features.map((feature, index) => {
                  const isImportantItem = feature.title === t('eventPackagesSection.packages.item2.title')
                  const importantItemClassname = isImportantItem ? 'text-secondary dark:text-secondary-foreground' : 'text-secondary/50 dark:text-secondary-foreground/50 '
                  return (
                  <TableRow key={index} className={cn("text-base sm:text-h7 lg:text-h6 xl:text-h4 font-h4 leading-14 md:leading-16 xl:leading-25 border-b-[1px] border-b-secondary/40 dark:border-b-secondary-foreground/40 hover:text-secondary dark:hover:text-secondary-foreground hover:bg-primary/5 dark:hover:bg-primary/10 transition-all duration-200", importantItemClassname)}>
                    <TableCell>
                      {feature.title}
                    </TableCell>
